...
{
    internal sealed class VicHURT : Missile
    {

        public static Color exhaustColor = new Color("f5ff00");

        [JsonConverter(typeof(StringEnumConverter))]
        public enum VicHURTType
        {
            Normal
        }

        [JsonProperty]
        public VicHURTType MissileType = VicHURTType.Normal;

        public override Spr? GetIcon()
        {
            return ModEntry.Instance.HURTSmall.Sprite;
        }

        public override string GetDialogueTag() => "VicHURT";
        ...
        public override List<Tooltip> GetTooltips()
        {
            List<Tooltip> tooltips = [
                new GlossaryTooltip($"{ModEntry.Instance.Package.Manifest.UniqueName}::{GetType()}")
                {
                    ...
                    Title = ModEntry.Instance.Localizations.Localize(["midrow", "HURT", MissileType.ToString(), "name"]),
                    TitleColor = Colors.midrow,
                    Description = ModEntry.Instance.Localizations.Localize(["midrow", "HURT", MissileType.ToString(), "description"])
                }
            ];
        ...
        public override void Render(G g, Vec v)
        {
            ...
            bool flag2 = false;
            Spr spr = ModEntry.Instance.HURTMidrow.Sprite;
