...
{
    internal sealed class VicHeavySeeker : Missile
    {

        public static Color exhaustColor = new Color("D70003");

    [JsonConverter(typeof(StringEnumConverter))]
        public enum VicHeavySeekerType
        {
            Normal
        }

        [JsonProperty]
        public VicHeavySeekerType MissileType = VicHeavySeekerType.Normal;

        public override Spr? GetIcon()
        {
            return ModEntry.Instance.HeavySeekerSmall.Sprite;
        }

        public override string GetDialogueTag() => "VicHeavySeekerTag";
        ...
        public override List<Tooltip> GetTooltips()
        {
            List<Tooltip> tooltips = [
                new GlossaryTooltip($"{ModEntry.Instance.Package.Manifest.UniqueName}::{GetType()}")
                {
                    ...
                    Title = ModEntry.Instance.Localizations.Localize(["midrow", "HeavySeeker", MissileType.ToString(), "name"]),
                    TitleColor = Colors.midrow,
                    Description = ModEntry.Instance.Localizations.Localize(["midrow", "HeavySeeker", MissileType.ToString(), "description"])
                }
            ];
